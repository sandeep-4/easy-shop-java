...
public class ProductQueryHelper {
	...
	public void prepareConditionListById(HashMap<Integer, Object> mapParameters, String data,
			MapParameterKey mapParametersKey, List<String> conditions, String field) {
	...
	public ParamsToQueryContext getParamsToQueryMap(HashMap<String, String> condotionMap) {
		if (condotionMap == null) {
			return null;
		}
		...
		for (Map.Entry<String, String> entry : condotionMap.entrySet()) {
			switch (QueryType.valueOf(entry.getKey())) {
			case genders:
				prepareConditionListById(mapParams, entry.getValue(), mapParametersKey, conditions,
						"p.genferCategory.id");
				applyFilter = true;
				break;
			case apparels:
				prepareConditionListById(mapParams, entry.getValue(), mapParametersKey, conditions,
						"p.apparelCategory.id");
				applyFilter = true;
				break;

			case brands:
				prepareConditionListById(mapParams, entry.getValue(), mapParametersKey, conditions,
						"p.productBrandCategory.id");
				applyFilter = true;
				break;

			case prices:
				prepareConditionListById(mapParams, entry.getValue(), mapParametersKey, conditions,
						"p.priceRangeCategory.id");
				applyFilter = true;
				break;
			case sortby:
			...
			case productname:
				prepareConditionListByName(mapParams, entry.getValue(), mapParametersKey, conditions, "p.name");
				break;
